...
public final class RepositorySerializerV3 implements RepositorySerializerType
{
  private static final String NAMESPACE = "urn:one.lfa.updater.repository.xml:3.0";
    ...
    final Repository repository)
  {
    ...
    for (final var pack : repository.items()) {
      if (pack instanceof RepositoryAndroidPackage) {
        root.appendChild(ofAndroidPackage(
          document,
          (RepositoryAndroidPackage) pack));
    ...
    final RepositoryOPDSPackage pack)
  {
    ...
    pack.password().ifPresent(pass -> {
      root.setAttribute("installPasswordSha256", hashOf(pass).text());
    });
    return root;
  }
  ...
  {
    return Hash.builder()
      .setText(digestText(text))
      .build();
  }

  private static String digestText(
    final String text)
  {
    try {
      final var digest =
